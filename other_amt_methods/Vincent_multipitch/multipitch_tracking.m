function multipitch_tracking(wavfile,f0file)

% MULTIPITCH_TRACKING Polyphonic note tracking on the MIDI scale using NMF
% under harmonicity and spectral smoothness constraints
%
% multipitch_tracking(wavfile,f0file)
%
% Inputs:
% wavfile: input audio filename
% f0file: output transcription filename (MIREX format with onset and offset
% in seconds and discrete pitch in Hz of each note)
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% Writing into a file
nbnotes=length(testnotes);
fid=fopen(f0file,'w');
for n=1:nbnotes-1,
    fprintf(fid,'%.3f\t%.3f\t%.2f\n',testnotes(n,5),testnotes(n,6),testnotes(n,3));
end
fprintf(fid,'%.3f\t%.3f\t%.2f',testnotes(end,5),testnotes(end,6),testnotes(end,3));
fclose(fid);
