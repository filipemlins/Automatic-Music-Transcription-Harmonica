...
%% INPUT SIGNAL
%x = wavread('C.wav');
x = wavread('/Users/schramm/GoogleDrive/PROJECTS/Musidata/PolyTranSingers/data/wav/vowels-004.wav');
%x = wavread('brent2.wav');
x = x(:); xlen = length(x);

%% COMPUTE COEFFIENTS
% full rasterized transform
Xcq = vqt(x, B, fs, fmin, fmax, 'rasterize', 'full','gamma', gamma);
...
switch(Xcq.rast)
    ...
    case 'piecewise'
        if strcmp(Xcq.format, 'sparse')
            cFill = vqtFillSparse(c,Xcq.M,Xcq.B);
